...
class Kpiw_Api {
    ...
    public function getData() {
        ...
        $query = 'SELECT ' . $dateField . ', ' . $agg . ' ';
        $query.= 'FROM ' . $metricTable . ' ';
        $query.= $join . ' ';
        $query.= 'WHERE ' . $where . ' ';
        $query.= 'GROUP BY ' . $dateField;
        ...
        if (isset($_GET['seg_fld'])) {
            ...
            $query = 'SELECT ' . $dateField . ' AS dat, ' . $agg . ' AS val, ' . $segField . ' AS seg ';
            $query.= 'FROM ' . $metricTable . ' ';
            $query.= $join . ' ';
            $query.= 'WHERE ' . $where . ' ';
            $query.= 'GROUP BY ' . $dateField . ', ' . $segField;
            $stmt = $this->_db->prepare($query);
            $stmt->execute($bind);
