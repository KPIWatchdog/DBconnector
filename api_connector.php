...
class Kpiw_Api {
    ...
    public function dbData() {
        ...
        $query = 'SELECT DATE(add_date), COUNT(*) ';
        $query.= 'FROM cms_articles ';
        $query.= 'WHERE DATE(add_date) >= :start AND DATE(add_date) <= :end ';
        $query.= 'GROUP BY DATE(add_date)';
    ...
    public function dbDataWithSegments() {
        ...
        $query = 'SELECT DATE(add_date) AS dat, category_name AS cat, COUNT(*) AS val ';
        $query.= 'FROM cms_articles ';
        $query.= 'INNER JOIN cms_categories USING (category_id) ';
        $query.= 'WHERE DATE(add_date) >= :start AND DATE(add_date) <= :end ';
        $query.= 'GROUP BY DATE(add_date), category_id';
        $stmt = $this->_db->prepare($query);
        $stmt->execute(array('start' => $_GET['start_date'], 'end' => $_GET['end_date']));
